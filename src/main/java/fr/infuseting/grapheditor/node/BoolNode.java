...
public class BoolNode extends Node{
    ...
    public BoolNode(int id, double x, double y) {
        this(id, false, x, y);
    }
    public BoolNode(int id, boolean value, double x, double y) {
        this(id, value, x, y, 200, 100);
    }
    public BoolNode(int id, boolean value, double x, double y, double width, double height) {
        super(id, "BoolNode", x, y, width, height);
        this.value = value;
        addPort(EnumPortType.OUTPUT, new Port(0, "BoolOutput", EnumPortType.OUTPUT, EnumDataType.BOOL, this, false));
    ...
    public void addInCanvas(Pane canva, Controller controller) {
        try {
            ...
            if (portsInput.isEmpty()) {
                ...
                input.setPromptText("Bool");
                input.setText(String.valueOf(this.value));

                input.focusedProperty().addListener((event -> {
                    if (!input.isFocused()) {
                        if (input.getText().equalsIgnoreCase("TRUE") || input.getText().equalsIgnoreCase("FALSE")) {
                            this.value = Boolean.parseBoolean(input.getText().strip());
                        } else {

                            controller.errorPopup("Invalid value", "Please enter a valid boolean value (true/false)", "Error");
                            input.setText(String.valueOf(this.value));
                            input.requestFocus();
                        }

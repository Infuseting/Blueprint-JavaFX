...
public class IntNode extends Node{
    ...
    public IntNode(int id, double x, double y) {
        this(id, 0, x, y);
    }
    public IntNode(int id, int value, double x, double y) {
        this(id, value, x, y, 200, 100);
    }
    public IntNode(int id, int value, double x, double y, double width, double height) {
        super(id, "IntNode", x, y, width, height);
        this.value = value;
        addPort(EnumPortType.OUTPUT, new Port(0, "StringOutput", EnumPortType.OUTPUT, EnumDataType.INT, this, false));
    }
    public int getValue() {
        return value;
    }
    ...
    public void addInCanvas(Pane canva, Controller controller) {
        try {
            ...
            if (portsInput.isEmpty()) {
                ...
                input.setPromptText("Integer");
                input.setText(String.valueOf(this.value));

                input.focusedProperty().addListener((event -> {
                    if (!input.isFocused()) {
                        if (input.getText().matches("-?\\d+")) {
                            this.value = Integer.parseInt(input.getText().strip());
                        } else {

                            controller.errorPopup("Invalid value", "Please enter a valid Integer value", "Error");
                            input.setText(String.valueOf(this.value));
                            input.requestFocus();
                        }
